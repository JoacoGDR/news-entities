...
exports.findSimilarFacts = findSimilarFacts;
exports.findSimilarTags = findSimilarTags;
exports.calculateCosineSimilarity = calculateCosineSimilarity;
exports.batchUpdateEmbeddings = batchUpdateEmbeddings;
/**
 ...
 */
async function findSimilarArticles(dataSource, targetEmbedding, limit = 10, threshold) {
 ...
 * @param threshold Optional distance threshold (lower = more similar)
 */
async function findSimilarFacts(dataSource, targetEmbedding, limit = 10, threshold) {
    if (targetEmbedding.length !== 1536) {
        throw new Error('Target embedding must be exactly 1536 dimensions');
    }
    const vectorString = `[${targetEmbedding.join(',')}]`;
 ...
 * @param threshold Optional distance threshold (lower = more similar)
 */
async function findSimilarTags(dataSource, targetEmbedding, limit = 10, threshold) {
    if (targetEmbedding.length !== 1536) {
        throw new Error('Target embedding must be exactly 1536 dimensions');
    }
    const vectorString = `[${targetEmbedding.join(',')}]`;
 ...
 */
async function calculateCosineSimilarity(dataSource, embedding1, embedding2) {
 ...
 */
async function batchUpdateEmbeddings(dataSource, tableName, embeddings) {
